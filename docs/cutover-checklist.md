...
|---|---|---|---|
| Ideas capture | ✅ Full (Convex-backed) | ✅ Full (localStorage + duplicate detection) | V2 drops Convex dependency for MVP |
...
| Calendar view | ✅ Postiz calendar | 🟡 Schedule dates visible in draft list | Full calendar UI is Postiz-owned; handoff is manual |
...
### Partial coverage
- **Calendar view**: Schedule dates are captured per draft but no calendar grid. Postiz calendar remains the scheduling UI during transition.
...
|---|---|---|---|
| Ideas | Convex DB | localStorage (per browser) | Re-enter top priority ideas manually; archive Convex data in JSON export |
| Posts/drafts | Convex DB | localStorage | No migration needed; start fresh in V2 |
| Voice packs | Hardcoded (lib/v2.ts) | Hardcoded defaults + localStorage edits | Expand if brand voice changes significantly |
| Research briefs | Not present in legacy | localStorage only (V2) | No migration needed |
...
| Post scheduling | **Postiz** | Copy approved draft text into Postiz scheduler |
...
2. V2 can be disabled by removing the `/v2` route or feature-flagging the nav link — no data loss because V2 uses localStorage only.
...
| View in cross-brand inbox | All-brands drafts toggle | ✅ |
...
- [ ] Convex data exported and archived
